...
export default function Magnetic({ children, strength = 0.28, className = '' }: { children: ReactNode; strength?: number; className?: string }) {
  ...
  return <motion.div ref={ref} onPointerMove={handleMove} onPointerLeave={handleLeave} style={{ x, y }} className={`will-change-transform inline-block ${className}`}>{children}</motion.div>;
}
